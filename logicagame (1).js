...
const Input = {
  ...
  set(x, y) { this.pressed = true; this.x = x; this.y = y; },
};

canvas.addEventListener('mousedown', (e) => {
  const r = canvas.getBoundingClientRect();
  Input.set(e.clientX - r.left, e.clientY - r.top);
});
canvas.addEventListener('touchstart', (e) => {
  e.preventDefault();
  const r = canvas.getBoundingClientRect(), t = e.touches[0];
  Input.set(t.clientX - r.left, t.clientY - r.top);
}, { passive: false });
...
class PowerUp {
  constructor() { this.reset(); }
  reset() { this.visible = false; this.x = 0; this.y = 0; this.type = ''; this.timer = 0; }
  show() {
    const types = Object.keys(POWERUP_TYPES);
    this.type    = types[Math.floor(Math.random() * types.length)];
    this.visible = true;
    this.x       = randomBetween(80, canvas.width - 80);
...
function updateGameplay() {
  ...
  if (!powerup.visible && frameCount >= gv.powerupNextAt) {
    powerup.show();
  }
